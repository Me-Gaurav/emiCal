...
const LogInForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        } else if (formValues.password !== decryptedPassword){
                        ...
                        <div className="w-full rounded-lg shadow border md:mt-0 sm:max-w-md xl:p-0 bg-gray-800 border-gray-700">
                            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                ...
                                <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                                    ...
                                    <div>
                                        <input type="password" value={formValues.password} name="password" onChange={handleInput} className="sm:text-sm rounded-lg  block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white outline-none focus:outline focus:outline-gray-500 focus:outline-1" placeholder="Password" required="" />
                                        {passwordError && <p className='text-[13px] text-red-400 -mb-4'>{passwordError}</p>}
                                    </div>
                                    <button type="submit" className="w-full text-white bg-sky-800 hover:bg-sky-900  focus:ring-1 focus:outline-none focus:ring-sky-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Sign In</button>
                                </form>
                            </div>
                        </div>
                    </div>
